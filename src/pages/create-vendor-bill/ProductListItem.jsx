	...
	return (
		<div className='flex flex-col items-start w-full' key={Math.random()}>
			...
			<div
				...
				{[
					{ label: 'Product', attr: 'productName', hsn: '4327599' },
					...
					<div className='flex flex-col justify-between items-center p-2 gap-y-2' key={Math.random()}>
						...
						{
							box.hsn && (<p className='text-gray-500 font-bold'>HSN: {box.hsn}</p>)
						}
					</div>
				))}
				<Paper className='p-4 bg-blue-200' sx={{ backgroundColor: 'rgba(0, 0, 255, 0.1)' }}>
					<div className='grid grid-cols-4 gap-12 gap-y-4'>
						<div className='flex flex-col justify-between items-center p-2 gap-y-2' key={Math.random()}>
							<p className='font-bold text-secondary-grey'>Batch No.</p>
							<p className='px-4 py-2 border border-border-gray rounded-lg bg-white'>{product.batchNo}</p>
						</div>
						<div className='flex flex-col justify-between items-center p-2 gap-y-2' key={Math.random()}>
							<p className='font-bold text-secondary-grey'>Rate</p>
							<p className='px-4 py-2 rounded-lg'>{product.rate}</p>
						</div>
						<div className='flex flex-col justify-between items-center p-2 gap-y-2' key={Math.random()}>
							<p className='font-bold text-secondary-grey'>Currency</p>
							<p className='px-4 py-2 rounded-lg'>{product.currency}</p>
						</div>
						<div className='flex flex-col justify-between items-center p-2 gap-y-2' key={Math.random()}>
							<p className='font-bold text-secondary-grey'>Amount</p>
							<p className='px-4 py-2 rounded-lg'>{product.amount}</p>
						</div>
					</div>
				</Paper>
			</div>
